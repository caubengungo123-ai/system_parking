	...
	#define LCD_COLS            16
	#define LCD_ROWS            2
	#define ADMIN_CARD_UID   {0x03, 0x04, 0xF6, 0x13}
		...
		for (;;) {
			...
			if (danger) {
				...
				/* Nhịp còi: ON 3 tick (300ms) - OFF 2 tick (200ms) */
						  ...
						  lcdCount++;
						  if (lcdCount >= 10) {          // 10 tick × 50ms = 500ms mới cập nhật 1 lần
							  lcdCount = 0;
	...
	static void Task_RFID(void *arg)
	{
		...
		for (;;) {
			...
			if (xSemaphoreTake(emergencySem, 0) == pdTRUE) {
				xSemaphoreGive(emergencySem);
				adminHolding = 0;
				vTaskDelay(pdMS_TO_TICKS(200));
				continue;
	...
	int main(void)
	{
		...
		if (lcdMutex == NULL || emergencySem == NULL) {
